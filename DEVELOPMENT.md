...
#### 核心功能
- **候选记忆证据化** - 候选记忆新增证据片段、AI 判断理由和置信度
- **编辑后记住** - 候选记忆支持修改内容、分类、重要性后再确认
- **来源追溯** - 候选记忆和长期记忆可跳转查看来源日记
- **自我画像反馈** - 画像记忆支持“像我 / 不准确”，不准确会停用对应长期记忆
- **相关记忆优先** - 新增 `BuildAIContextUseCase`，AI 对话时优先注入与当前问题相关的长期记忆

#### 数据库迁移
- 版本 8 → 9：`ai_memory_candidates` 新增 `evidence`、`reason`、`confidence` 字段

#### 已知限制
- 相关记忆匹配仍为关键词/文本包含策略，暂未引入 embedding 或语义检索
- 画像反馈复用长期记忆启停机制，暂未新增独立反馈表或版本历史
